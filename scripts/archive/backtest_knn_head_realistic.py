...
class Scenario:
    ...
    def roundtrip_cost_pct(self) -> float:
        return 2.0 * (self.fee_bps_side + self.slippage_bps_side) / 100.0
...
def _class_metrics(rows: list[dict[str, object]], *, buy_thr: float, sell_thr: float) -> dict[str, float | int]:
...
def _simulate_strategy(
    ...
    *,
    ...
    for idx, query in enumerate(records):
        ...
        reason = "cooldown" if cooldown_until is not None and query.day < cooldown_until else "eligible"
        ...
        decision_rows.append(
            {
                ...
                "eligible_to_trade": reason == "eligible" and signal != "HOLD",
                "eligibility_reason": reason,
                "evidence": evidence,
            }
        )

        if cooldown_until is not None and query.day < cooldown_until:
            continue
        if signal == "HOLD":
            continue

        side = 1.0 if signal == "BUY" else -1.0
        gross_trade_return = side * actual_ret_7d
        net_trade_return = gross_trade_return - scenario.roundtrip_cost_pct
        exit_day = query.day + timedelta(days=hold_days)
        cooldown_until = exit_day

        trade_rows.append(
            {
                "entry_date": query.date,
                "exit_date": exit_day.isoformat(),
                "signal": signal,
                "gross_return_pct": round(gross_trade_return, 4),
                "net_return_pct": round(net_trade_return, 4),
                "actual_return_7d": round(actual_ret_7d, 4),
    ...
    class_summary = _class_metrics(decision_rows, buy_thr=config.buy_thr, sell_thr=config.sell_thr)
    trade_returns = [float(row["net_return_pct"]) for row in trade_rows]
    wins = [ret for ret in trade_returns if ret > 0.0]
    losses = [ret for ret in trade_returns if ret <= 0.0]
    ...
    return {
        ...
        "trading_summary": scenario_summary,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Realistic-ish offline backtest with fixed 7d hold, long/short/flat, "
            "roundtrip costs, and retrieved evidence rows."
        )
    )
    ...
    parser.add_argument("--output", default="artifacts/backtests/knn_head_realistic_backtest.json")
