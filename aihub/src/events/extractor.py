...
class EventExtractor:
    ...
    async def _llm_extract(self, request: EventExtractionRequest) -> list[EventRecord]:
        ...
        prompt = (
            ...
            f"{summary_part}\n\n"
            "For each event provide event_group (one of: Regulation & Legal, Macroeconomic, "
            "Protocol & Product, Market Performance, Whale & On-chain, Risk & Warning, "
            "Industry Standards & Opinions, Technology & Development, Sentiment & Narrative), "
            "event_type (specific type within group), polarity (float -1 to 1), "
            "confidence (float 0 to 1), entities (list of strings).\n\n"
