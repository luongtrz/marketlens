...
const LightweightChart: React.FC<LightweightChartProps> = ({
    ...
    useEffect(() => {
        ...
        const chart = createChart(chartContainerRef.current, {
            ...
            timeScale: {
                ...
                tickMarkFormatter: (time: number, tickMarkType: any, locale: string) => {
                    const date = new Date(time * 1000);
                    // For daily or longer ticks, just show date
        ...
        chart.subscribeCrosshairMove((param: any) => {
            ...
            if (
                ...
                const mainData = mainSeriesRef.current ? param.seriesData.get(mainSeriesRef.current) : undefined;
                const volumeData = volumeSeriesRef.current ? param.seriesData.get(volumeSeriesRef.current) : undefined;
                
                if (mainData) {
                    onCrosshairMoveRef.current({
                        time: param.time,
                        open: mainData.open !== undefined ? mainData.open : mainData.value,
                        high: mainData.high !== undefined ? mainData.high : mainData.value,
                        low: mainData.low !== undefined ? mainData.low : mainData.value,
                        close: mainData.close !== undefined ? mainData.close : mainData.value,
        ...
        // Scroll to left edge detection
        const handleLogicalRangeChange = (logicalRange: any) => {
            if (logicalRange && logicalRange.from !== null && logicalRange.from < 5) {
                // We are near the left edge of the chart
                if (onLoadMoreRef.current && !loadingMoreRef.current) {
                    loadingMoreRef.current = true;
                    // Trigger load more with the oldest timestamp available in the series
                    // The actual callback implementation should handle fetching and resetting
                    // this debounce flag after data is loaded.
                    if (mainSeriesRef.current) {
                        const data = mainSeriesRef.current.data();
                        if (data && data.length > 0) {
                            onLoadMoreRef.current(data[0].time * 1000);
                        }
                    }
                    // Simple timeout fallback to prevent spam
                    setTimeout(() => {
    ...
    useEffect(() => {
        ...
        // Fit content on: 1) Initial load ONLY
        // Preserve user's zoom when toggling indicators or changing chart type
        ...
        // Always update refs to avoid infinite loops or stale state
        if (typeChanged) prevTypeRef.current = type;
        if (indicatorsChanged) prevIndicatorsRef.current = indicators;
