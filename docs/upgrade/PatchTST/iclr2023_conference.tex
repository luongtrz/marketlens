...
%Patch Time Series Transformer for Long-term Forecasting and Representation Learning

%Channel Independence with Patching: Time Series Forecasting with Transformers

%PatchTST: Efficient Transformer for Time Series Forecasting and Representation Learning

%PatchTST: Effective Transformer for Time Series Forecasting and Representation Learning

%Transformers for Long-term Series Forecasting
%Patch Transformers for Time Series Forecast
%Transformers for Time Series Forecasting and Representation Learning
}
...
%This paper proposes a simple design of Transformer model for multivariate time series forecasting. The design is based on two key components: (i) decomposition of time series into different subsequence patches which are served as input tokens to Transformer; (ii) channel independence where each channel contains a single univariate time series that shares the same embedding and Transformer weights across all the series. Patching design naturally has three-fold benefit: local information is retained in the embedding; computation and memory usage of the attention maps is quadratically reduced given the same look-back window; and the model can attend longer history. This seemingly uncomplicated structure can improve the forecasting significantly, often by a large margin up to $50 \%$ when comparing with SOTA Transformer models. We apply our channel independence patch time series Transformer (PatchTST) to self-supervised pre-training task and attain excellent fine-tuning performance, often outperform supervised training on large datasets. Masked pre-training on one dataset and transferring to other datasets also produces state-of-the-art forecasting accuracy.

%We propose an efficient design of Transformer models for multivariate time series forecasting and self-supervised representation learning. The design is based on two key components: (i) segmentation of time series into subseries-level patches which are served as input tokens to Transformer; (ii) channel-independence where each channel contains a single univariate time series that shares the same embedding and Transformer weights across all the series. Patching design naturally has three-fold benefit: local semantic information is retained in the embedding; computation and memory usage of the attention maps are quadratically reduced given the same look-back window; and the model can attend longer history. Our channel-independent patch time series Transformer (PatchTST) can improve the long-term forecasting accuracy significantly when compared with that of SOTA Transformer-based models. We also apply our model to self-supervised pre-training tasks and attain excellent fine-tuning performance, which often outperforms supervised training on large datasets. Masked pre-training on one dataset and transferring to other datasets also produces SOTA forecasting accuracy. \gift{rephrase to: Transferring of masked pre-training performed on one dataset to other datasets also produces SOTA forecasting accuracy}

We propose an efficient design of Transformer-based models for multivariate time series forecasting and self-supervised representation learning. It is based on two key components: (i) segmentation of time series into subseries-level patches which are served as input tokens to Transformer; (ii) channel-independence where each channel contains a single univariate time series that shares the same embedding and Transformer weights across all the series. Patching design naturally has three-fold benefit: local semantic information is retained in the embedding; computation and memory usage of the attention maps are quadratically reduced given the same look-back window; and the model can attend longer history. Our channel-independent patch time series Transformer (PatchTST) can improve the long-term forecasting accuracy significantly when compared with that of SOTA Transformer-based models. We also apply our model to self-supervised pre-training tasks and attain excellent fine-tuning performance, which outperforms supervised training on large datasets. Transferring of masked pre-trained representation on one dataset to others also produces SOTA forecasting accuracy.



% Over the last few years, there has been several proposed Transformer architectures for long-range multivariate time series forecasting. While these models show superior forecasting performance over sequential methods such as LSTM and ARIMA, the recently proposed simple DLinear model challenges the effectiveness of these complex Transformer models. In this paper, we affirmatively answer that question. We show that by considering time series patches instead of individual time steps as Transformer input tokens, together with weight sharing design across different series, we can achieve significantly better forecasting performance than the state-of-the-art Transformer models and better than DLinear model under the same settings. Furthermore, our multi-channel patch Transformer (PatchTST) can be applied to pretrained tasks and attain excellent forecasting performance when transferring to downstream tasks. \yuqi{One of my personal thought: although we are inspired by DLinear paper, but maybe we don't need to emphasize it too much. We could just introduce our work and mention it in the benchmarks.} \nam{AAA}
\end{abstract}
...
\begin{table*}[t]
	...
	\scalebox{0.7}{
		\begin{tabular}{ccccccc}
            ...
            \midrule
			%&\multirow{1}*{FEDFormer}& Mix channels & 0.201 \\            
...
%In the following sections, we will introduce our approach in more details. We conduct extensive experiments to demonstrate the model effectiveness and provide ablation studies to indicate the importance of both patching and channel-independence components. 
...
%First, masking is applied at each time step. Since temporal redundancy often occurs in time series data, the masked values at the current time step can be referred from neighboring values without , which may make the reconstruction process trivial and thus the representation may not carry important abstract information. \citet{tst} proposed complex randomization strategies to resolve the problem.

% Since time series is often temporally redundant, the masked values at the current time step can be easily referred from neighboring values, which makes the reconstruction process trivial and thus the representation may not carry important abstract information.
...
%thus the results would be more stable and convincing than the other smaller datasets since they are less affected by the randomness in training (e.g. initial condition, stochastic batching).
...
%\textbf{Baselines and Experimental Settings.} We use DLinear \citep{dlinear}, FEDformer \citep{fedformer}, Autoformer \citep{autoformer}, Informer \citep{informer}, Pyraformer \citep{pyraformer}, and LogTrans \citep{logtrans} as our baselines. All of the models are following the same experimental setup with prediction length $T\in \{24, 36, 48, 60\}$ for ILI dataset and $T\in \{96, 192, 336, 720\}$ for other datasets as in the original papers. We mainly cite their results from \citet{dlinear} with the default look-back window $L=96$ for Transformer-based models, and $L=336$ for DLinear. But in order to avoid under-estimating the baselines, we also run FEDformer, Autoformer and Informer for six different look-back window $L\in \{24, 48, 96, 192, 336, 720\}$, and always choose the best results to create strong baselines. More details about the baselines could be found in Appendix \ref{append::baseline}. We calculate the MSE and MAE of multivariate time series forecasting as metrics. 
...
%TS2Vec extracts both temporal and contextual information via hierarchical contrastive loss while BTSF encodes the representation from both time and frequency domains. 
...
%Furthermore, Table \ref{tab:ablation} shows how patching help reducing the memory consumption. We have already seen in Table \ref{table::channel independece vs mixing} that the running time without patching is significantly higher. Another interesting observation is that when the number of channels is large, like Traffic and Electricity datasets, channel-independence also contributes to model size reduction, primarily due to the fact that it doesn't need the large projection matrix from all channels to the hidden space. More details about the ablation studies on patching and channel-independence can be found in the Appendix.
...
%Informer often overfit the data as $L$ increases while FEDformer and Autoformer stay flat. 
